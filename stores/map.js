  ...
  actions: {
    async fetchData() {
      try {
        ...
        let data = await this.scrollPage(page, ".m6QErb[aria-label]", 10);
        this.mapsData = data;
        console.log('Fetched data:', this.mapsData);
        await browser.close();
      } catch (error) {
        console.error('Error fetching data:', error);
      }
    },
    async scrollPage(page, scrollContainer, itemTargetCount) {
      let items = [];
    ...
    async extractItems(page) {
      let mapsData = await page.evaluate(() => {
